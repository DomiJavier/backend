...
app.post('/login', (req, res) => {
  ...
  connection.query(query, (error, results) => {
    ...
    if (results.length > 0) {
      ...
      bcrypt.compare(password, results[0].pass, function(err, response) {
        if (err){
...
app.post('/recuperar', (req, res) => {
  ...
  bcrypt.hash(password, salt, (err, hash) => {
    ...
    const query = `UPDATE usuarios SET pass='${hash}' WHERE usuario='${usuario}'`;
    console.log(`usuario perdida`);
    connection.query(query, (error, results) => {
      if (error) {
        console.log(`Usuario = ${usuario} incorrecto`);
        res.json({ success: false, message: 'Credenciales incorrectas' });
      } else {
        console.log(`Usuario = ${usuario} ha recuperado la contraseña`);
        res.json({ success: true, message: 'Ha recuperado la contraseña' });
      }  
  });
});
});
...
app.post('/HistoricoMedicacion', (req, res) => {
  const { usuario } = req.body;
  const fechaC = new Date();
  const mes = fechaC.getMonth();
  const fecha = `${fechaC.getFullYear()}-${meses[mes]}-${fechaC.getUTCDate()}`;
